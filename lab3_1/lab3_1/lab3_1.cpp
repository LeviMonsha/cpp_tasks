...
void query_address_info(void* addr) {
    MEMORY_BASIC_INFORMATION info;
    if (!VirtualQuery(addr, &info, sizeof(info))) return;
    show_info(info);
}
...
{
    int local_var = 0;
    void* addr = (void*)0; //null
    void* addr_change = addr;
    MEMORY_BASIC_INFORMATION info;
    ...
    cout << setw(8) << "Address" << " "
        ...
        << setw(8) << "Protect" << " "
        << endl;
    query_address_info((void*)0);
    query_address_info(main);
    query_address_info(&global_var);
    query_address_info(&local_var);
    cout << "success" << endl;
    cout << endl;

    cout << "Additional task:" << endl;
    const uint32_t mb = 1024 * 1024;
    LPVOID memory = VirtualAlloc(NULL, //базовый адрес
                                mb, //размер
                                MEM_COMMIT, //способ получения
                                PAGE_EXECUTE_READ | PAGE_READWRITE //тип доступа
                                );
    query_address_info(memory);
}
